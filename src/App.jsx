...
/* ── Moonshot API Helper ── */
async function callMoonshot(apiKey, messages, maxTokens = 1000) {
  ...
  if (!res.ok) {
    ...
    throw new Error(err.error?.message || `Moonshot API error: ${res.status}`);
  }
...
export default function App() {
  ...
  const generate = useCallback(async () => {
    if (!apiKey) { setError("Please enter your Moonshot API key above."); return; }
    if (!type || !pillar) return;
    
    setLoading(true); setError(null); setResult(null); setImgUrl(null);
    try {
      const raw = await callMoonshot(apiKey, [
        { role: "system", content: sysPrompt() },
        { role: "user", content: userPrompt(type, pillar, topic, extra) }
      ], 1000);
  ...
  }, [apiKey, type, pillar, topic, extra]);

  const generateDaily = useCallback(async () => {
    if (!apiKey) { setDailyError("Please enter your Moonshot API key above."); return; }
    ...
    try {
      ...
      const calls = seeds.map((seed, i) =>
        callMoonshot(apiKey, [
          { role: "system", content: dailySysPrompt() },
          { role: "user", content: dailyUserPrompt(seed, DAILY_FORMATS[i], usedThemes) }
        ], 1200)
        .then(raw => {
          const parsed = JSON.parse(raw);
          return {
  ...
  return (
    <div style={s.root}>
      <header style={s.header}>
        <div style={s.hRow}>
          <div style={s.logo}>
            ...
            <div>
              ...
              <div style={s.logoSub}>Content Engine · Moonshot AI</div>
            </div>
          </div>
          <div style={s.stats}>
            {[
        ...
        <div style={s.keyRow}>
          <div style={s.keyLabel}>🔑 Moonshot API Key</div>
          <div style={s.keyWrap}>
            <input
          ...
          <div style={s.keyHint}>Get yours at platform.moonshot.ai · Key stays in your browser only</div>
        </div>
        ...
        {tab === "generate" && (
          <div style={s.panel}>
            ...
            <button
              style={{
                ...s.genBtn,
                opacity: !type || !pillar || !apiKey ? 0.5 : 1,
                cursor: !type || !pillar || !apiKey ? "not-allowed" : "pointer",
              }}
              onClick={generate}
              disabled={!type || !pillar || loading}
            >
              {loading ? "✨ Generating with Kimi..." : "⚡ Generate Post"}
            </button>
        ...
        {tab === "daily" && (
          <div style={s.panel}>
            <div style={s.dailyHeader}>
              ...
              <div style={s.dailySub}>Generates 4 unique posts in parallel using Kimi K2.6</div>
            </div>
            ...
            <button
              ...
              disabled={dailyLoading || !apiKey}
            >
              {dailyLoading ? "✨ Cooking 4 posts with Kimi..." : "📅 Generate Daily Plan (×4)"}
            </button>
...
const s = {
  ...
  keyLabel: { fontSize: 12, fontWeight: 600, color: "#374151", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" },
  keyWrap: { display: "flex", gap: 8 },
  keyInput: {
    flex: 1,
    padding: "10px 14px",
    borderRadius: 8,
  ...
  keyHint: { fontSize: 11, color: "#9ca3af", marginTop: 6 },
